...
function plot( $xml_filename, $jpeg_filename )
{
  ...
  {
    fatal_error( sprintf( 'Input file "%s" is not a valid XML file', $xml_filename ), 10 );
  }
  ...
  if( false === $match )
  {
    fatal_error( sprintf( 'Expected XML structure not found in input file "%s"', $xml_filename ), 11 );
  }
  ...
  foreach( $group_xml as $group_key => $group_values )
  {
    ...
    foreach( $group_values['keys'] as $data_key )
    {
      ...
      if( false === $match )
      {
        fatal_error( sprintf( 'Expected XML structure not found in input file "%s"', $xml_filename ), 12 );
      }
